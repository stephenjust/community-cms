...
var urlBase = "./scripts/adminupdate.php";
var formVars = "";
var changing = false;
   ...
   var spans = document.getElementsByTagName("span");
	...
	for (var i=0; i<spans.length; i++){
		var spn = spans[i];
        	...
        	if (((' '+spn.className+' ').indexOf("news_title") != -1) && (spn.id)) {
			spn.onclick = function () { editBox(this); }
			spn.style.cursor = "pointer";
			spn.title = "Click to edit title";	
       		}
       	if (((' '+spn.className+' ').indexOf("news_content") != -1) && (spn.id)) {
			spn.onclick = function () { editBox(this); }
			spn.style.cursor = "pointer";
			spn.title = "Click to edit content";	
       		}
...
function addEvent(elm, evType, fn, useCapture)
{
  if (elm.addEventListener){
    ...
    alert("To use the administration functions on this page, you must enable JavaScript in your browser.");
  }
}
