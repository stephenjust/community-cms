...
class CalCategory {
	...
	public function __construct($id) {
		$result = DBConn::get()->query(sprintf('SELECT `label`, `colour` FROM `%s` '
	...
	function delete() {
		...
		DBConn::get()->query(sprintf('DELETE FROM `%s` WHERE `cat_id` = :id',
	...
	public static function count() {
		$result = DBConn::get()->query(sprintf('SELECT COUNT(*) AS `count` FROM `%s`',
				CALENDAR_CATEGORY_TABLE), null, DBConn::FETCH);
		return $result['count'];
	}
