...
switch($event_block->attribute['mode']) {
	default:
		...
		break;
	case 'upcoming':
		$upcoming_event_query = 'SELECT * FROM ' . CALENDAR_TABLE . '
		...
		$upcoming_event_handle = $db->sql_query($upcoming_event_query);
		if($db->error[$upcoming_event_handle] === 1) {
			$bl_return .= 'Failed to load upcoming dates.<br />';
			return $bl_return;
		}
		if($db->sql_num_rows($upcoming_event_handle) == 0) {
